...
{
    public MiniBikeRenderer(EntityType<MiniBikeEntity> type)
    {
    ...
    protected void render(@Nullable MiniBikeEntity vehicle, PoseStack matrixStack, MultiBufferSource renderTypeBuffer, float partialTicks, int light)
    {
        ...
        this.renderDamagedPart(vehicle, VehicleModels.MINI_BIKE_BODY, matrixStack, renderTypeBuffer, light, partialTicks);
        this.renderSteeringWheel(vehicle, VehicleModels.MINI_BIKE_HANDLES, 1.0, 0.75, -3.3375, 0.7F, -19F, matrixStack, renderTypeBuffer, light, partialTicks);
    ...
    @Override
    public void applyPlayerRender(MiniBikeEntity entity, Player player, float partialTicks, PoseStack matrixStack, VertexConsumer builder)
    {
        int index = entity.getSeatTracker().getSeatIndex(player.getUUID());
        if (index != -1)
        {
